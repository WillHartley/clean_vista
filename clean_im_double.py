...
class video_image:
    ...
    def read_mask(self):
        ...
        segmap[segmap==1] = 2 # why is this like this?
        segmap[segmap==0] = 1
        segmap[segmap>1] = 0
        ...
        if self.conf.bug_check == True:
            self.save_bugcheck_im(segmap, "expanded_seg.fits")
    ...
    def fix_px(self, px_val, data, weights):
        # weighted mean. Masked pixels have weight zero.
        try:
    ...
    def clean_im(self):
        ...
        cleaned_im_tmp = np.zeros((self.im_size[0], self.im_size[1]))
        ...
        for i in range(self.im_size[0]):
            for j in range(self.im_size[1]):
                ...
                cleaned_im_tmp[i,j] = self.fix_px(self.im_data[i+self.conf.pad_px-1,j+self.conf.pad_px-1],
                                                       self.im_data[i+self.conf.pad_px-1-self.conf.filt_size[0]:i+self.conf.pad_px-1+self.conf.filt_size[0]+1,j+self.conf.pad_px-1-self.conf.filt_size[1]:j+self.conf.pad_px-1+self.conf.filt_size[1]+1],
                                                       self.mask[i+self.conf.pad_px-1-self.conf.filt_size[0]:i+self.conf.pad_px-1+self.conf.filt_size[0]+1,j+self.conf.pad_px-1-self.conf.filt_size[1]:j+self.conf.pad_px-1+self.conf.filt_size[1]+1])

	# second pass - with filter indicies reversed
        # embed the temporary cleaned image in buffer
        cleaned_im_tmp = self.pad_array(cleaned_im_tmp)
        for i in range(self.im_size[0]):
            for j in range(self.im_size[1]):
                # the slices are of padded arrays, and extend the number of pixels given
                # by the half-size of the filter, in self.conf.filt_size, either side of
                # the target pixel. Slice dimension is always odd.
                self.cleaned_im[i,j] = self.fix_px(cleaned_im_tmp[i+self.conf.pad_px-1,j+self.conf.pad_px-1],
                                                       cleaned_im_tmp[i+self.conf.pad_px-1-self.conf.filt_size[1]:i+self.conf.pad_px-1+self.conf.filt_size[1]+1,j+self.conf.pad_px-1-self.conf.filt_size[0]:j+self.conf.pad_px-1+self.conf.filt_size[0]+1],
                                                       self.mask[i+self.conf.pad_px-1-self.conf.filt_size[1]:i+self.conf.pad_px-1+self.conf.filt_size[1]+1,j+self.conf.pad_px-1-self.conf.filt_size[0]:j+self.conf.pad_px-1+self.conf.filt_size[0]+1])
        del cleaned_im_tmp
    ...
    def save_clean_im(self):
        ...
        hdu.writeto(self.clean_name, overwrite=True)
...
if __name__ == '__main__':
    ...
    conf.filt_size = [100, 1] # half-dimensions of the rectangular filter
    conf.border = 300 # depth in pixels that forms mask region around the border
    conf.clip = 500. # 
    conf.bug_check = True # do we want to save bug checking images?
